...
callback = tf.keras.callbacks.ModelCheckpoint(filepath="fashion_mnist/checkpoints_more_features/",
                                                verbose=1,
                                                save_best_only=True,
                                                save_freq='epoch')
    ...
    while True:
        ...
        for i in range(batch_size):
            if index >= len(data_set_list[0]):
                index = 0
            batchX.append(data_set_list[0][index].reshape((28,28,1)))
...
class CustomCallback(tf.keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs=None):
        ...
        for key in keys:
            s += f"{key}: {logs[key]:.4f} - "
        print(s[:-3], file=open('fashion_mnist_more_features.txt', 'a'))
...
class Net():
    def __init__(self, input_shape):
        ...
        self.model.add(layers.Conv2D(10, 5, input_shape = input_shape, activation = 'relu'))
        ...
        self.model.add(layers.Conv2D(100, 3, activation = 'relu'))
    ...
    def print_summary(self, summaryStr):
        print(summaryStr)
        print(summaryStr, file=open('fashion_mnist_more_features.txt', 'a'))
...
tf.keras.models.save_model(net, "fashion_mnist/saves_more_features/")
